...
If step 2 *passes*, first check the injected import actually made it into the cruised graph — the summary's dependency count should have gone up by one versus the clean run. An import that never enters the graph (e.g. a type-only import cruised without `tsPreCompilationDeps: true`) passes every rule without being checked, which looks identical to a working config.
