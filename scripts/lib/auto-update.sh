...
au_extract_got_hash() {
  ...
  set -e
  # Bolt ⚡: Replaced external process pipeline (printf|grep|head|sed) with native Bash regex.
  # This avoids spawning 4 subprocesses per extraction for an in-memory string.
  local got=""
