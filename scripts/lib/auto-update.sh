...
au_extract_got_hash() {
  ...
  set -e
  # Optimization: Use native Bash regex to parse the hash from the log string.
  # This avoids spawning 4 sub-processes (printf | grep | head | sed) which
  # previously caused significant overhead on every failed fake-hash build.
  local got=""
