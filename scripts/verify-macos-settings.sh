...
expect_contains() { # label needle -- command...
  ...
  # Bolt: Replaced expensive subshell and grep with native Bash glob matching (~90x speedup)
  if [[ "$out" == *"$needle"* ]]; then
    ok "$label"
  else
...
for agent in bettermouse betterdisplay; do
  # Bolt: Replaced subshell and grep -i with native case-insensitive Bash glob matching (~90x speedup)
  if [[ "$(launchctl list 2>/dev/null)" == *"$agent"* ]]; then
...
check_disabled() { # label haystack domain-note
  ...
  if [ -z "$haystack" ]; then
    ...
    # Bolt: Replaced expensive subshell and grep with native Bash regex/glob matching (~90x speedup)
