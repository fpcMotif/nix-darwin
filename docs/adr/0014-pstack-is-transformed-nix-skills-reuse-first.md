...
The global catalog is code-dev-first (ADR-0017) and every catalog entry costs context on every session. Several pstack skills duplicate skills already installed: mattpocock `tdd`, `teach`, `prototype`, `wait-what`, `writing-for-agents`, `diagnosing-bugs`; the built-in `simplify`; ripwire's edit check; the CLAUDE.md comment and writing rules.
