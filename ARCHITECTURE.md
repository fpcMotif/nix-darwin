...
| CI                   | GitHub Actions (`.github/workflows/build.yml`). CI does not run `nix flake check`. The `flake-check` job builds every `checks.x86_64-linux` attribute on Ubuntu; the `darwin` job builds every `checks.aarch64-darwin` attribute on macOS. Both read the names with `builtins.attrNames`, as `just check` does, and `unit-build-workflow` fails if the workflow names a check by hand. The `darwin`, `nixos`, and `nixos-aarch64` jobs wait for `flake-check`, then build active Darwin (after its checks and the source-build guard), the x86_64 NixOS scaffold (`x230`), and `vm-aarch64-utm`. Also builds `devShells` on both runners and the `dev-container` image for both Linux systems. |
