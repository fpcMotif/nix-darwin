...
| CI                   | GitHub Actions (`.github/workflows/build.yml`): builds active Darwin, x86_64 NixOS scaffolds (`wsl`, `x230`), and `vm-aarch64-utm` on macOS / Ubuntu runners, gating config builds on the `ci-gate` check group. Check-group membership has one writer — `tests/default.nix`'s `groups` attrset, exposed as the `checkGroups.<system>` flake output — resolved to `nix build` attrpaths by `scripts/lib/check-group.sh`; `justfile`'s `check` recipe (`quick`) and `auto-update.yml`'s nightly guard (`nightly-guard`, kept IFD-safe — see Linting row) enumerate their own named subsets from the same source instead of hardcoding attr lists. Also builds `devShells` on both runners and the `dev-container` image for both Linux systems. |
